...
const Card = ({className, children}: CommonComponentProps) => {
  return (
    <div className={`flex items-center gap-2 sm:gap-3 md:justify-start ${className}`}>
      {children}
    </div>
    )
};
...
const IconContainer = ({className, children}: CommonComponentProps) => {
  return (
    <div className={`size-8 sm:size-10 rounded-lg bg-accent flex items-center justify-center shrink-0 ${className}`}>
      {children}
    </div>
  )
};
